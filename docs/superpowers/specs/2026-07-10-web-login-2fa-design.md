...
挑战令牌有效期固定为 5 分钟。它不代表已登录状态，不能访问任何需要管理员会话的接口。TOTP 时间计数器防重放和恢复码原子消费用于阻止同一个第二因素凭据重复完成挑战。
...
- 每次生成 10 个恢复码。
- 每个恢复码使用密码学安全随机数，编码时排除易混淆字符并分组展示。
- 输入比较前忽略大小写、空格和连字符。
...
- 限流器设置容量上限和过期清理，避免攻击者通过大量随机用户名造成无界内存增长。
...
二维码由前端根据服务端返回的 `otpauth_uri` 生成；手动密钥始终作为无二维码环境的备用方式。
